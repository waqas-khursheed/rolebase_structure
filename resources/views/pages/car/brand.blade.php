      ...
      <div class="container-fluid">
        <div class="row">
            <div class="col-12">
              ...
              <!-- form start -->
                <form role="form" id="quickForm" method="POST" action="{{URL::to('brand')}}" enctype="multipart/form-data">
                    @csrf
                    <div class="card-body">
                        <div class="form-group ">
                            <input type="text" class="form-control" placeholder="Brand Name" name="brand_name" required="" >
                            @if ($errors->has('brand_name'))
                                <label id="brand_name-error" class="error" for="brand_name">{{ $errors->first('brand_name') }}</label>
                            @endif
                        </div>
                        <div class="form-group ">
                ...
                <tbody>
                    @foreach ($brand as $item)
                        <tr>
                            ...
                            <td>
                                <img src="{{asset($item->brand_image)}}" alt="{{asset($item->brand_image)}}"  style="border-radius: 10%;" width="100" height="100">
                            </td>
                            <td>
                        ...
                        <div class="modal fade" id="delete{{$item->id}}" tabindex="-1" role="dialog" aria-labelledby="delete{{$item->id}}Label" aria-hidden="true">
                                ...
                                <div class="modal-content">
                                    ...
                                    <form id="car-form" action="{{ URL::to('brand/'.$item->id) }}" Method="post" style="display: none;">
                                    @csrf
                                    @method('DELETE')
                            ...
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body"> 
                                        <div class="container">
                                            <form id="form_validation" method="POST" action="{{URL::to('brand/'.$item->id)}}" enctype="multipart/form-data">
                                                @csrf
                                                @method('PUT')
                                                <div class="form-group form-float mt-4">
                                                    <input type="text" class="form-control" placeholder="Brand Name" name="brand_name" value="{{$item->brand_name}}"> 
                                                </div>
                                                <div>
